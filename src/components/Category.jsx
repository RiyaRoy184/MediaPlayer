...
const Category = ({ setDeleteResponseFromCategory,deleteResponseFromView }) => {
  ...
  const videoCardDropOverCategory = async (e, categoryDetails) => {
    ...
    // update category by adding video to it's allVideos
        ...
        {/* single category view */}
        {
          allCategories?.length > 0 ?
            allCategories?.map(categoryDetails => (
              <div droppable="true" onDragOver={dragOverCategory} onDrop={e =>videoCardDropOverCategory(e,
                categoryDetails)} key={categoryDetails?.id} className="border rounded p-3 mb-3">
                ...
                <div className="row mt-2">
                  {
                    ...
                    categoryDetails?.allVideos?.map(video => (
                      <div draggable={true} onDragStart={e=>categoryVideoDragStarted
                        (e,video,categoryDetails)}
      ...
      >
        ...
        <Modal.Body>
          <FloatingLabel controlId="floatingCategoryName" label="Category Name">
            <Form.Control onChange={e => setCategoryName(e.target.value)} type="text"
